...
3. **대한체육회(ksoc)**: 별도 API가 없으므로 공개된 공고 게시판만 접근하고,
   ...
   금지). `docs/compliance-ksoc.md`에 사람이 직접 확인한 robots.txt/저작권
   정책 검토 기록이 있다 — 현재 수집 경로(`/sports/` 하위)는 robots.txt상
   허용된 것으로 확인됐으므로, 이 문서의 결론과 일치하면 매번 무조건
   중단하지 않는다. 다만 이 저장소는 `sports.or.kr`에 직접 접근할 수 없는
   네트워크 정책 하에 있을 수 있으므로, robots.txt를 직접 확인할 수 있는
   상황이라면 매 실행 시 재확인해서 `docs/compliance-ksoc.md`의 결론과
   달라졌는지 대조하고, 달라졌으면 즉시 중단해 보고한다. 확인 자체가
   불가능한 환경이면(이 문서 작성 시점의 세션처럼) 마지막 확인 기록을
   신뢰하되 그 사실을 산출물에 명시한다. `/sports/`, `/business/` 경로
   밖으로 수집 대상을 넓히는 경우 이 문서의 결론이 자동 적용되지 않으니
   반드시 별도로 재검토를 요청한다.
