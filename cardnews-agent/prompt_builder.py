"""헤드라인·페르소나 → DALL-E 배경 이미지 프롬프트 변환 (Gemini 2단계).
...
def build_dalle_prompt(headline: str, persona: str) -> str:
    """헤드라인+페르소나를 조합해 최종 DALL-E 프롬프트(영어)를 생성한다."""
    scene = llm_client.generate_text(SCENE_PROMPT.format(headline=headline))
    style = llm_client.generate_text(STYLE_PROMPT.format(persona=persona))
    return (
