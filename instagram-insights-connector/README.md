...
**연령대별/성별 선호도**는 `follower_demographics` API로 가져오는 **계정 전체 팔로워의
연령×성별 분포 스냅샷**입니다. Instagram API는 "이 게시물에 어떤 연령/성별이 반응했는지"를
알려주는 게시물 단위 인구통계를 제공하지 않으므로, 콘텐츠별 교차분석이 아니라 계정 전체
참고 수치라는 점에 유의하세요. `daily-instagram-brief.ps1`이 매일 실행 시 이 지표를 함께
가져와 `data/instagram.json`의 `followerDemographics` 필드에 저장합니다. 팔로워 수가
Meta의 최소 요건에 못 미치는 등 조회에 실패하면(치명적 오류로 처리하지 않고) 이전 값을
유지하며, 데이터가 아예 없으면 대시보드에 "데이터 연동 준비 중" 빈 상태가 표시됩니다.
